...
def update_status(step, steps=100, description=None):
    ...
    current_task.update_state(
        ...
        meta=schemas.Status(
            progress=step,
            totalAmount=steps,
            description=f"Working {step/steps:%}"
...
def process_video(video=schemas.VideoRequest) -> schemas.Result:
    ...
    try:
        ...
        video_file, audio_file = download_video(video.url, target_path=dump_path)
        update_status(5, description="Extracting slides")
        slides = extract_and_save_slides(video_file, dump_path=dump_path)
        ...
        res = schemas.Status(
            ...
            status="finished",
            result=schemas.VmarkdownDocument(vmarkdown=vmarkdown),
        )
